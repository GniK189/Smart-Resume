  ...
  const deleteResume = async (resumeId) => {
    try {
      ...
      if (confirm) {
        ...
        setAllResumes(allResumes.filter((resume) => resumeId !== resume._id));
        toast.success(data.message);
      }
    } catch (error) {
      toast.error(error?.response?.data?.message || error.message);
          ...
          >
            ...
            >
                ...
                >
                  ...
                  <div className="flex flex-col items-center justify-center gap-2 border group text-slate-400 border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-indigo-500 hover:text-indigo-700 cursor-pointer transition-colors">
                    {resume ? (
                      ...
                      <>
                        <UploadCloud className="size-14 strock-1" />
                        <p>Upload Resume</p>
                      </>
                    )}
                  </div>
              ...
              <button
                disabled={isLoading}
                type="submit"
                className="flex items-center justify-center gap-2 w-full py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition-colors"
              >
                {isLoading && (
